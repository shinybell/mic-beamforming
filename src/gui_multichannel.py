...
def update_filters():
    ...
    if nyquist > audio_state["high_pass_cutoff"]:
        ...
        if 0 < norm_cutoff < 1:
            audio_state["hp_b"], audio_state["hp_a"] = signal.butter(
                4, norm_cutoff, btype="high", analog=False
            )
        else:
    ...
    for bs_filter in audio_state["band_stop_filters"]:
        if nyquist > bs_filter["high"] and bs_filter["low"] < bs_filter["high"]:
            ...
            if 0 < low < high < 1:
                bs_filter["b"], bs_filter["a"] = signal.butter(
                    4, [low, high], btype="bandstop", analog=False
                )
            else:
...
class AudioApp:
    def __init__(self, root):
        ...
        style.configure(
            "TButton", background=self.panel_bg, foreground=self.fg_color, borderwidth=1
        )
        ...
        self.canvas_scroll = tk.Canvas(
            canvas_frame, bg=self.bg_color, highlightthickness=0
        )
        scrollbar = ttk.Scrollbar(
            canvas_frame, orient="vertical", command=self.canvas_scroll.yview
        )
        ...
        gain_frame = ttk.LabelFrame(
            self.scrollable_frame, text="Master Gain", padding="10"
        )
        ...
        ttk.Scale(
            ...
            from_=10,
            to=1000,
            variable=self.hp_freq,
            command=self.on_hp_change,
        ).pack(fill=tk.X)
        ...
        ttk.Label(
            bs_header_frame, text="Band Stop Filters", font=("Helvetica", 12, "bold")
        ).pack(side=tk.LEFT)
        ttk.Button(
            bs_header_frame, text="+ Add Filter", command=self.add_filter_ui
        ).pack(side=tk.RIGHT)
    ...
    def add_filter_ui(self, initial_freqs=None):
        ...
        frame = ttk.LabelFrame(
            self.filters_container, text=f"Filter #{fid + 1}", padding="10"
        )
        ...
        del_btn = ttk.Button(
            top_row, text="Remove", command=lambda: self.remove_filter(fid, frame)
        )
        ...
        low_scale = ttk.Scale(
            frame, from_=100, to=SAMPLE_RATE // 2 - 100, variable=low_var
        )
    ...
    def remove_filter(self, fid, frame_widget):
        # Remove from data
        audio_state["band_stop_filters"] = [
            f for f in audio_state["band_stop_filters"] if f["id"] != fid
        ]
        # Remove from UI
    ...
    def start_audio(self):
        ...
        self.daq_thread = threading.Thread(
            target=daq_thread_func, args=(self.stop_event,), daemon=True
        )
