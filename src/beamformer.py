...
class BeamformerBase(ABC):
    ...
    def _calculate_delays(self, theta_deg):
        ...
        return (
            np.dot(self.mic_positions, np.array([ux, uy, uz])) / config.SPEED_OF_SOUND
        )
...
class DelayAndSumBeamformer(BeamformerBase):
    def __init__(
        ...
        self.steering_vector = np.ones(
            (self.num_bins, self.num_mics), dtype=np.complex64
        )
...
class MVDRBeamformer(BeamformerBase):
    def __init__(
        ...
        self.R = np.zeros(
            (self.num_bins, self.num_mics, self.num_mics), dtype=np.complex64
        )
        for k in range(self.num_bins):
            self.R[k] = (
                np.eye(self.num_mics, dtype=np.complex64) * 10.0
            )  # Larger init to prevent instability

        # Steering vector a(f, theta) - pure delay vector, NOT weights
        self.steering_vector_a = np.ones(
            (self.num_bins, self.num_mics), dtype=np.complex64
        )
    ...
    def apply(self, multichannel_chunk, debug=False):
        ...
        if debug:
            ...
            print(
                f"  [MVDR Debug] Input range: [{np.min(multichannel_chunk):.6f}, {np.max(multichannel_chunk):.6f}]"
            )
            print(f"  [MVDR Debug] Spectrum shape: {spectrum.shape}")
            print(
                f"  [MVDR Debug] Spectrum magnitude range: [{np.min(np.abs(spectrum)):.6f}, {np.max(np.abs(spectrum)):.6f}]"
            )
        ...
        R_loaded = (
            self.R + np.eye(num_mics, dtype=np.complex64)[np.newaxis, :, :] * 1e-6
        )
        ...
        if debug:
            print(
                f"  [MVDR Debug] Denominator range: [{np.min(np.abs(denom)):.6e}, {np.max(np.abs(denom)):.6e}]"
            )
            print(
                f"  [MVDR Debug] R matrix diagonal mean: {np.mean(np.abs(np.diagonal(self.R, axis1=1, axis2=2))):.6e}"
            )
        ...
        if debug:
            print(
                f"  [MVDR Debug] w_mvdr magnitude range: [{np.min(np.abs(w_mvdr)):.6f}, {np.max(np.abs(w_mvdr)):.6f}]"
            )
        ...
        if debug:
            print(
                f"  [MVDR Debug] Beamformed spectrum magnitude range: [{np.min(np.abs(beamformed_spectrum)):.6f}, {np.max(np.abs(beamformed_spectrum)):.6f}]"
            )
        ...
        if debug:
            print(
                f"  [MVDR Debug] Output range: [{np.min(beamformed_chunk):.6f}, {np.max(beamformed_chunk):.6f}]"
            )
...
def process_wav_file(
    ...
    if file_sample_rate != sample_rate:
        print(
            f"Warning: File sample rate ({file_sample_rate} Hz) differs from config ({sample_rate} Hz)"
        )
    ...
    print(
        f"Loaded {num_samples} samples ({duration_sec:.2f} seconds) at {sample_rate} Hz"
    )

    # Create beamformer
    if beamformer_type == "mvdr":
        beamformer = MVDRBeamformer(
            mic_positions=mic_positions, sample_rate=sample_rate, chunk_size=chunk_size
        )
    else:
    ...
    for start in range(0, num_samples, chunk_size):
        ...
        if num_chunks % 100 == 0:
            ...
            print(
                f"  Processing: {progress:.1f}%, Output range so far: [{min_output:.3f}, {max_output:.3f}]"
            )
